...
class Genetic:
    def __init__(self, n_population, victims_list, wall_list, tlim):
        ...
        self.victim_dict_coordinate = self.generate_victims_dict(victims_list)
        # exit()
    ...
    def genesis(self):
        ...
        for i in range(self.n_population):
            ...
            sol_i = np.append(np.append([0], victim_list[np.random.choice(list(range(self.n_victims)), self.n_victims, replace=False)]),[0])
            population_set.append(sol_i)
        return np.array(population_set)

    def generate_victims_dict_distances(self):
        ...
        for victim in self.victim_dict_coordinate.keys():
            ...
            self.compute_distances_from_victim(victim)

    def compute_distances_from_victim(self, victim):
        cont = 0
                ...
                if child[0] not in self.walled_positions + explored + [n[0] for n in frontier]:
                    for vic, vic_info in self.victim_dict_coordinate.items():
                        if child[0] == vic_info["coord"]:
                            ...
                            self.victim_dict_distances[victim][vic] = {"trajectory": child[1], "cost": self.calculate_trajectory_cost(child[1])}
                            cont +=1
                            # print(cont)
    ...
    def generate_victims_dict(self, victims_list):
        victim_dict = {}
        i = 1
    ...
    def fitness_eval(self, victim_list):
        total = 0
        points = 0
        ...
        if total >= self.TLIM:
            del victim_list[-2]
            return self.fitness_eval(victim_list)
    ...
    def get_all_fitnes(self):
        fitnes_list = np.zeros(self.n_population)

        # Looping over all solutions computing the fitness for each solution
        # print(self.population)
        for i in range(self.n_population):
            fitnes_list[i] = self.fitness_eval(self.population[i])

        # print(fitnes_list)

        return fitnes_list

    def progenitor_selection(self):
        ...
        # pprint(progenitor_list_a)
        # pprint(progenitor_list_b)
        # print(np.array([progenitor_list_a, progenitor_list_b]))
